...
// Build 方法用于构建DFA，它会将提供的所有单词添加到DFA中。
func (d *DFAMatcher) Build(words []string) {
	for _, item := range words { // 遍历提供的所有单词。
		cleanedWord := strings.Replace(item, "(! @#$%^&*()_+)", "", -1)
		fmt.Println(cleanedWord)
		d.root.AddWord(strings.ToLower(item)) // 将每一个单词添加到DFA的根节点。
	}
}
...
// FindChild 方法用于在当前节点的子节点中查找一个特定的子节点。

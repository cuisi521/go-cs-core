...
func Marshal(v interface{}) (marshaledBytes []byte, err error) {
	...
	if err != nil {
		err = errors.New(err.Error() + `json.Marshal failed`)
	}
	return
}
...
func MarshalIndent(v interface{}, prefix, indent string) (marshaledBytes []byte, err error) {
	...
	if err != nil {
		err = errors.New(err.Error() + `json.MarshalIndent failed`)
	}
	return
}
...
func Unmarshal(data []byte, v interface{}) (err error) {
	...
	if err != nil {
		err = errors.New(err.Error() + `json.Unmarshal failed`)
	}
	return
}
...
func UnmarshalUseNumber(data []byte, v interface{}) (err error) {
	...
	if err != nil {
		err = errors.New(err.Error() + `json.UnmarshalUseNumber failed`)
	}
	return
}
